...
XML;
	...
	#[Test]
	public function parse_git_status_porcelain_counts_staged_modified_and_untracked(): void
	{
		$raw = " M src/Foo.php\nA  src/New.php\nMM src/Both.php\n?? src/Untracked.php";
		$result = McpShell::parseGitStatusPorcelain($raw);

		$this->assertFalse($result['clean']);
		$this->assertCount(4, $result['files']);
		$this->assertSame('staged:2 modified:2 untracked:1', $result['summary']);
	}

	#[Test]
	public function parse_git_status_porcelain_handles_clean_worktree(): void
	{
		$result = McpShell::parseGitStatusPorcelain('');

		$this->assertTrue($result['clean']);
		$this->assertSame([], $result['files']);
		$this->assertSame('Working tree clean', $result['summary']);
	}
}
