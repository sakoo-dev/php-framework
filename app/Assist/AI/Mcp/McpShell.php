...
final class McpShell
{
	...
	public function gitStatusParsed(string $path = ''): array
	{
		...
		$staged = count(array_filter($files, static fn (array $f): bool => !str_starts_with($f['status'], '?') && !str_starts_with($f['status'], ' ')));
		$untracked = count(array_filter($files, static fn (array $f): bool => '??' === $f['status']));
