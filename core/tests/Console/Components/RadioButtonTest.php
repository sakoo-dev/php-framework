...
final class RadioButtonTest extends TestCase
{
	...
	{
		$this->assertInstanceOf(RadioButton::class, new RadioButton('Select an option', ['Option 1', 'Option 2']));
	}
	...
	#[Test]
	public function fallback_mode_method_exists_and_is_accessible(): void
	{
		$radio = new RadioButton('Choose one:', ['Apple', 'Banana', 'Cherry']);
		$reflection = new \ReflectionClass($radio);

		$this->assertTrue($reflection->hasMethod('fallbackMode'));
		$this->assertTrue($reflection->getMethod('fallbackMode')->isPrivate());
	}
	...
	public function clear_lines_generates_correct_escape_sequences(): void
	{
		...
		$output = $this->captureOutput(fn () => $method->invoke($radio, 3));

		$this->assertStringContainsString("\033[1A", $output);
	...
	public function render_output_format(): void
	{
		...
		$output = $this->captureOutput(fn () => $method->invoke($radio));
	...
	{
		$radio = new RadioButton('What is your choice?', ['Yes', 'No']);
		$reflection = new \ReflectionClass($radio);
		$property = $reflection->getProperty('prompt');
		$property->setAccessible(true);

		$this->assertEquals('What is your choice?', $property->getValue($radio));
	}
	...
	public function render_with_different_selected_index(): void
	{
		...
		$selectedProp->setValue($radio, 1);

		$renderMethod = $reflection->getMethod('render');
		$renderMethod->setAccessible(true);

		$output = $this->captureOutput(fn () => $renderMethod->invoke($radio));
